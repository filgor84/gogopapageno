...
/*
lexer contains the file data, the starting point inside the input
		...
		for true {
			...
			if curStateIndex == -1 {
				if lastFinalStateReached == nil {
					...
					//fmt.Printf("%s: %d\n", text, ruleNum)
					absPos := l.getAbsPosOfSymbol(startPos, l.pos)
					result = lexerFunction(thread, ruleNum, text, absPos, genSym)
					break
				}
			} else {
...
func (l *lexer) getAbsPosOfSymbol(relStart, relEnd int) *position {
	absStart := l.startPosInInput + relStart
	absEnd := absStart + relEnd - relStart - 1
	return newPosition(absStart, absEnd)
}
...
*/
func lex(threadNum int, data []byte, startPoint int, pool *stackPool, c chan lexResult) {
	start := time.Now()
