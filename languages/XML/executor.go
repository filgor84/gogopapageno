...
//AgainstFile specify the path of the file against which
//the XPath query has to be executed
...
func (executorCommand *executorCommandImpl) AgainstFile(filePath string) ExecutorCommand {
	executorCommand.documentFilePath = filePath
	return executorCommand
}

func (executorCommand *executorCommandImpl) AgainstString(input []byte) ExecutorCommand {
	executorCommand.inputString = input
	return executorCommand
}
...
//Go takes care of executing the command and to return
func (executorCommand *executorCommandImpl) Go() (results []Position, err error) {
	...
	executor.setDocumentToBeParsedFilePath(executorCommand.documentFilePath)
	executor.input = executorCommand.inputString
	...
	err = executor.executeUDPEsWhileParsingDocumentFile()
	if err != nil {
		return
	}
...
func (executor *executorImpl) setDocumentToBeParsedFilePath(documentFilePath string) {
	executor.documentFilePath = documentFilePath
}
...
func (executor *executorImpl) executeUDPEsWhileParsingDocumentFile() error {

	var axiomSymbol *symbol
	var err error

	if len(executor.input) > 0 {
		axiomSymbol, err = ParseString(executor.input, executor.numberOfThreads)
	} else {
		axiomSymbol, err = ParseFile(executor.documentFilePath, executor.numberOfThreads)
	}

	if err != nil {
		return fmt.Errorf("unable to execute query while parsing document %q with %d threads: %v", executor.documentFilePath, executor.numberOfThreads, err)
	}
	executor.resultingExecutionTable = axiomSymbol.Value.(nonTerminal).executionTable()
	return err
}
