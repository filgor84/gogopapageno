...
func ParseString(str []byte, numThreads int) (*symbol, error) {
	...
	stackPoolBaseSize := 4 * math.Ceil(4*(((float64(rawInputSize)/avgCharsPerToken)/float64(_STACK_SIZE))/float64(numThreads)))
	stackPtrPoolBaseSize := 4 * math.Ceil(4*((float64(rawInputSize)/avgCharsPerToken)/float64(_STACK_PTR_SIZE))/float64(numThreads))
